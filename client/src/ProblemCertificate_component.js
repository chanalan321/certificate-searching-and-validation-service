...
function createData(modified,createdAt,studentName, courseName, schoolName, courseDate,_id) {
  return { modified,createdAt,studentName, courseName, schoolName, courseDate, _id };
}

export default function ProblemCertificate_component() {
  ...
  const handleSubmit = (event,_id) => {
    ...
    fetch('/modifyCertificate', {
      method: 'POST',
      headers: {
      'Accept': 'application/json',
      'Content-Type': 'application/json'
      },
      body: JSON.stringify({textData: textData,studentLName:studentLName, studentFName: studentFName,studentID: studentID,courseName:courseName ,courseDate:finishDate})})
      .then(res => res.json())
      .then(setOpen(false)&setSearched(false)&setSnackbarOpen(true)&setSnackbarMessage(_id))
  }; 
  ...
  useEffect(() => {        
    if (!searched) {        
      ...
      fetch('/searchDB', {
        ...
        body: JSON.stringify({textData: textData, searchType: searchType})})
        .then(res => res.json())      
        .then(res => res.forEach(function(obj) { rows_n.push(createData(obj.modified,obj.createdAt,obj.studentName, obj.courseName, obj.schoolName, obj.courseDate, obj._id)) }) )
  ...
  return(
    ...
    <Grid container md={12} justify="center" alignItems="center" className={classes.paper}>
        <Grid item md={11}>
    <div>
        <div><h1>Problem Certificate</h1></div>
      ...
      >
        ...
        <DialogContent className={classes.Dialog}>
          <form className={classes.form} noValidate onSubmit={(event) => handleSubmit(event, _id)}>
      ...
      <Snackbar
        ...
        message={"Certificate Modified Successfully Id: " + snackbarMessage}
        action={
          <React.Fragment>
